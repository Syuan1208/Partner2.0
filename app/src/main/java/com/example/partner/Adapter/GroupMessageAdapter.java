...
public class GroupMessageAdapter extends RecyclerView.Adapter<GroupMessageAdapter.ViewHolder> {
    ...
    public GroupMessageAdapter(Context mContext, List<Chat> mChat, ArrayList<String> mID, ArrayList<String> mID_Name, String imageURL, String classname,ArrayList<String> all_announcement) {
        this.mChat = mChat;
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        for (int i = 0; i < mID.size(); i++) {
            if (chat.getSender().equals(mID.get(i))) {
                holder.username.setText(mID_Name.get(i));
                DatabaseReference reference=FirebaseDatabase.getInstance(Url).getReference("Users").child(mID.get(i));
                        ...
                        public boolean onMenuItemClick(MenuItem menuItem) {
                            ...
                            switch (menuItem.getItemId()) {
                                case R.id.announcement:
                                    setannouncement(firebaseUser.getUid(), classname, show_message.getText().toString());
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for(DataSnapshot dataSnapshot:snapshot.getChildren()){
                    ...
                    if(chat.getReceiver().equals(classname)){
                        ...
                        int maxChars = 15; // 您想要限制的字符數量
                        if(thelastannouncement.length()>maxChars){
                            thelastannouncement = thelastannouncement.substring(0, maxChars) + "...";
                        }
                    }
                }
